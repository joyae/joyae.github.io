...
> * 위 두 계층은 __im2col(이미지를 행렬로 전개하는 함수)__ 을 이용하면 효율적으로 구현할 수 있다.
...
* CNN에서는 __합성곱 계층(Convolutional Layer)__ 와 __풀링 계층(Pooling Layer)__ 가 추가된다.
...
* 완전연결 계층의 문제점은 __'데이터의 형상이 무시'__ 된다는 것
...
* 필터를 적용하는 위치의 간격을 __스트라이드__ 라고 한다.
...
* 3차원 합성곱 연산에서 주의할 점은 __입력 데이터의 채널 수__ 와 __필터의 채널 수__ 가 같아야 한다는 점이다.
